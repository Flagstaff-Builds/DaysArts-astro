 ...
 * 
 * This script will:
 * 1. Find all movie MDX files
 * 2. Check if their descriptions are over 160 characters
 * 3. If so, truncate them to 160 characters (at word boundaries)
 * 4. Update the file with the new description
 */
 ...
 * Ensures truncation happens at word boundaries and adds ellipsis
 */
...
async function main() {
  try {
    // Find all MDX files
    const files = glob.sync(path.join(CONTENT_DIR, '**/*.mdx'));
